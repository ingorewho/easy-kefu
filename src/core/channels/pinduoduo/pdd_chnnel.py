...
from core.channels.pinduoduo.pdd_message import PDDChatMessage
from core.channels.channel import Channel
from core.channels.pinduoduo.utils.API.get_token import GetToken
...
class PDDChannel(Channel):
    def __init__(self, max_concurrent_messages: int = 50):
        ...
        # 非工作时间队列管理器
        from utils.business_hours_queue import business_hours_queue
        self.queue_manager = business_hours_queue
        self._queue_processor_registered = False

    async def _process_queued_message(self, queued_msg):
        """
        处理队列中的消息（在工作时间执行）

        Args:
            queued_msg: QueuedMessage 对象
        """
        try:
            self.logger.info(
                f"处理队列消息: msg_id={queued_msg.msg_id}, "
                f"user={queued_msg.nickname}, shop={queued_msg.shop_id}"
            )

            context = queued_msg.context
            queue_name = queued_msg.queue_name

            # 标记消息来自队列，避免被 BusinessHoursHandler 再次缓存
            context.kwargs['_from_business_hours_queue'] = True

            # 将消息放入正常处理队列
            from Message import put_message
            await put_message(queue_name, context)

            self.logger.debug(f"队列消息已放入处理队列: {queue_name}")
    ...
    async def _setup_queue_manager(self):
        """设置非工作时间队列管理器"""
        try:
            if not self._queue_processor_registered:
                # 注册消息处理器
                self.queue_manager.register_processor(self._process_queued_message)
                self._queue_processor_registered = True
                self.logger.info("已注册非工作时间队列处理器")
            ...
            self.logger.error(f"设置队列管理器失败: {e}")

    async def _cleanup_queue_manager(self):
        """清理队列管理器"""
        try:
            await self.queue_manager.stop()
            self.logger.info("非工作时间队列管理器已停止")
        except Exception as e:
            self.logger.error(f"清理队列管理器失败: {e}")
    ...
    async def _setup_message_consumer(self, queue_name: str):
        ...
        """
        try:
            from core.messages.message_consumer import message_consumer_manager
            from core.messages.message_handler import handler_chain
    ...
    async def _cleanup_resources(self, queue_name: str):
        ...
        try:
            ...
            # 清理非工作时间队列管理器
            await self._cleanup_queue_manager()

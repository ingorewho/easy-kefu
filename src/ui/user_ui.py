...
from core.channels.pinduoduo.pdd_login import login_pdd
    ...
    def onAddAccount(self):
        ...
        if not license_manager.is_valid():
            QMessageBox.warning(
                self, 
                "未授权", 
                f"软件未授权或授权已失效。\n{license_manager.get_error_message()}\n\n"
                f"请前往 设置 > 软件授权管理 导入有效的 License。"
            )
            return
        
        # 检查账号数量限制
        current_count = len(self.accounts_data)
        allowed, max_accounts = license_manager.check_account_limit(current_count)
        if not allowed:
            QMessageBox.warning(
                self,
                "账号数量超限",
                f"当前 License 允许的最大账号数为 {max_accounts} 个，\n"
                f"已达到上限。如需添加更多账号，请联系管理员升级 License。"
            )
            return
